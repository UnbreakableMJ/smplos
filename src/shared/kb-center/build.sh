...
# kb-center build helper for Arch Linux
...
if [[ "$action" == "--install" ]]; then
  echo "==> Installing to /usr/local/bin/kb-center"
  sudo install -Dm755 "$BIN_PATH" /usr/local/bin/kb-center
  echo "Installed: /usr/local/bin/kb-center"
elif [[ "$action" == "--run" ]]; then
  echo "==> Running kb-center"
  exec "$BIN_PATH"
else
