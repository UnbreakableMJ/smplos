    ...
    if let Ok(home) = std::env::var("HOME") {
        ...
        if legacy.exists() {
            match std::fs::remove_file(&legacy) {
                Ok(_) => debug_log!("[kb-center] removed legacy config {:?}", legacy),
                Err(e) => eprintln!("[kb-center] failed to remove legacy config {:?}: {}", legacy, e),
            }
        }
    }
}
...
pub fn load_from_os_config(available: &[AvailableLayout]) -> Option<Vec<ActiveLayout>> {
    ...
    debug_log!("[kb-center] load_from_os_config: trying {:?}", path);
    ...
    if result.is_empty() {
        ...
        debug_log!("[kb-center] load_from_os_config: {} layouts", result.len());
        Some(result)
    }
}
...
pub fn load_from_hyprland(available: &[AvailableLayout]) -> Option<Vec<ActiveLayout>> {
    debug_log!("[kb-center] load_from_hyprland: querying hyprctl...");
    ...
    debug_log!("[kb-center] hyprland: layouts={:?} variants={:?}", layout_str, variant_str);
...
/// Works on X11, Wayland (via XWayland), Plasma, GNOME, etc.
pub fn load_from_system(available: &[AvailableLayout]) -> Option<Vec<ActiveLayout>> {
    debug_log!("[kb-center] load_from_system: setxkbmap -query");
    ...
    debug_log!("[kb-center] system: layouts={:?} variants={:?}", layout_str, variant_str);
    if layout_str.is_empty() {
        return None;
    }
...
/// Detects Wayland (Hyprland) vs X11 (DWM) at runtime.
...
pub fn sync_to_compositor(active: &[ActiveLayout]) {
    ...
    if std::env::var("WAYLAND_DISPLAY").is_ok() {
        debug_log!("[kb-center] sync: hyprctl {} {}", layouts, variants);
        hyprctl_keyword("input:kb_layout", &layouts);
        hyprctl_keyword("input:kb_variant", &variants);
    } else {
        debug_log!("[kb-center] sync: setxkbmap {} {}", layouts, variants);
...
fn update_input_conf(layouts: &str, variants: &str) {
    ...
    match std::fs::write(&path, &result) {
        Ok(_) => debug_log!("[kb-center] update_input_conf: OK"),
        Err(e) => eprintln!("[kb-center] update_input_conf failed: {}", e),
    }
}

/// Detect current layouts from the running compositor.
/// Tries Hyprland first on Wayland, setxkbmap on X11.
pub fn load_from_compositor(available: &[AvailableLayout]) -> Option<Vec<ActiveLayout>> {
    debug_log!("[kb-center] load_from_compositor: wayland={}", std::env::var("WAYLAND_DISPLAY").is_ok());
    if std::env::var("WAYLAND_DISPLAY").is_ok() {
